...
public class SocketIoClient implements LicodeSignalingService {
    ...
    public void sendOfferSdp(String sdp,IStreamDescription streamDescription) {
        try {
           ...
           // wrapperJson.put
            ...
            wrapperJson.put("connectionId", streamDescription.getConnectionId());
            wrapperJson.put("erizoId", streamDescription.getErizoId());
            params.put("options",wrapperJson);
            mSocket.emit("connectionMessage", params, null);
    ...
    private void connectToRoomInternal(RoomConnectionParameters parameters) {
     ...
     //   VLog.d("Connect to room: " + parameters.host);
       ...
       // opts.query = "tokenId=" + parameters.tokenId +"signature=" + parameters.signature;
...
//                    }
                    ...
                    JSONObject rootObject = result.getJSONObject(0);
                    if(!rootObject.has("msg")) {
                        VLog.e("can't find msg obj" );
                        return;
                    }
                    JSONObject jsonObject = rootObject.optJSONObject("msg");
                    ...
                    if (jsonObject.has("iceServers")) {
                        ...
                        if (iceServers != null) {
                            for (int i = 0; i < iceServers.length(); i++) {
                                ...
                                String password = obj.optString("credential");
                                if (!TextUtils.isEmpty(url)) {
                                    tokenParams.iceServers.add(new PeerConnection.IceServer(url,username,password));
                                }
                            }
                        }
                    }
    ...
    private void sendPublish() throws JSONException {
        ...
        publishJson.put(" encryptTransport",true);
        ...
        metaData.put("type","publisher");
        publishJson.put("metadata", metaData);
        publishJson.put("minVideoBW", 0);
        publishJson.put("state","erizo");
        param.put("options",publishJson);
        param.put("sdp",null);
        mSocket.emit("publish", param,new Ack() {
            @Override
            public void call(Object... args) {
                ...
                publishParams.id = jsonArray.optLong(0);  //streamid
                publishParams.erizoId = jsonArray.optString(1); //erizoctrollerId
                publishParams.connectionId = jsonArray.optString(2); // connectid = erizeId_clientid
                mLocalStreamId = publishParams.id;
                try {
                    JSONObject jsonObject = new JSONObject();
                    jsonObject.put("id", publishParams.id);
                    JSONObject attrObject = new JSONObject();
                    attrObject.put("connectionId", publishParams.connectionId);
                    attrObject.put("erizoId", publishParams.erizoId);
                    jsonObject.put("attributes",attrObject);
    ...
    private void sendSubscribe(IStreamDescription params) throws JSONException {
        ...
        param.put("options",subscribeJson);
        VLog.d("sendSubscribe: "  + param);
        mSocket.emit("subscribe", param, null, new Ack() {
            @Override
            public void call(Object... args) {
    ...
    private void subscribeLicodeEvents() {
        mSocket.on("connection_message_erizo", new Emitter.Listener() {

            @Override
            public void call(Object... args) {
                JSONArray jsonArray = transformJsonArray(args);
                VLog.i("connection_message_erizo: "+jsonArray);

               // try {

                    JSONObject jsonObject = jsonArray.optJSONObject(0);
                    JSONObject msgObject = jsonObject.optJSONObject("msg");
                    JSONObject evtObject = msgObject.optJSONObject("evt");
//                    boolean answer = "answer".equals(mess.optString("type"));
//                    if (!answer) {
//                        VLog.i("expected ANSWER, got: " + mess.getString("type"));
//                        return;
//                    }
                    String sdp = evtObject.optString("sdp");
                    String connectionId = msgObject.optString("connectionId");
                   // long streamId = 0;

//                    if (msg.has("streamId")) {
//                        streamId =msg.optLong("streamId");
//                    } else {
//                        streamId = msg.optLong("peerId");
//                    }

                    if (events != null) {
                        events.onRemoteDescription(connectionId,sdp);
                    }
